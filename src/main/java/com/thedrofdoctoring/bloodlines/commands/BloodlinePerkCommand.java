        ...
        return Commands.literal("perk_points")
                .requires(context -> context.hasPermission(PERMISSION_LEVEL_CHEAT))
                        .then(Commands.argument("points", IntegerArgumentType.integer(0))
                                .executes(context -> addPerkPoints(context, IntegerArgumentType.getInteger(context, "points"), Lists.newArrayList(context.getSource().getPlayerOrException())))
                                .then(Commands.argument("player", EntityArgument.entities())
                                    .executes(context -> addPerkPoints(context, IntegerArgumentType.getInteger(context, "points"), EntityArgument.getPlayers(context, "player")))));
    }
